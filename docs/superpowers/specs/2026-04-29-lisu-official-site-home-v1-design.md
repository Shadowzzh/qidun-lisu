...
- 首页框架、布局节奏和站点壳优先参考 `ai-website-cloner-template/src` 现有实现，尽量保留页面骨架
- 不继承旧品牌内容模型，但允许以旧页面组件为基础做内容替换型改造
...
- 用户已明确要求尽量保留现有页面框架、布局和 section 节奏，因此实现策略应优先做内容替换，而不是重组页面骨架
...
- 首页实现优先保持旧模板的页面框架、布局关系和主要 section 节奏
...
- 先建立一个干净的独立官网工程
- 迁移少量通用工程配置和组件拆分思路
- 以旧模板首页和站点壳为布局参考，优先保持页面骨架
- 先完成首页需要的内容模型，再把内容映射回接近原布局的区块
- 再按 section 完成内容替换型实现
...
- 页面组件命名允许沿用模板的边界习惯，但内容模型必须改为北京骊甦科技版本
...
- 首页按 section 组织的写法
- 少量无品牌耦合的基础 UI 组件
- 首页 `home-page.tsx` 的页面框架、区块顺序和主要布局关系
...
- 8 个 section 的落位应尽量映射到旧模板首页已有的区块骨架
- 若旧模板某个区块可以通过替换文案、图片和卡片内容完成适配，则不应重做布局
- 只有当旧区块无法承载目标内容时，才允许做最小结构调整
